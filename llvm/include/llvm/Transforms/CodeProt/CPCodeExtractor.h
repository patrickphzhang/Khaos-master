...
class Value;
  ...
  public:
    ...
    void findInputsOutputs(ValueSet &Inputs, ValueSet &Outputs,
		  const ValueSet &Allocas, bool InputsOnly = false) const;
  ...
  private:
    ...
    Function *constructFunction(ValueSet &inputs,
                                ValueSet &outputs,
                                BasicBlock *header,
	...
	// CodeProt
	void findCPI(SmallVector<CatchPadInst*, 8> &vec);
	void findLRIAndRFP(SmallVector<CatchPadInst*, 8> &CPIvec,
		SmallVector<CallInst*, 8> &LRIvec,
		SmallVector<CallInst*, 8> &RFPvec);
	std::vector<CallInst*> findRecoverFp();
	AllocaInst* findEscapeAlloca(CallInst* callInst);
